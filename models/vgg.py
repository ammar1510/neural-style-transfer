...
class Vgg16(torch.nn.module()):
    "only those layers are exposed which have been proven to work well"
    def __init__(self, requires_grad=False, show_progress=False):
        ...
        def forward(self, x):
            ...
            x=self.slice4(x)
            relu4_3= x
            vgg_outputs= namedtuple("Vgg_outputs", self.layer_names)
            out = vgg_outputs(relu1_1, relu2_2, relu3_3, relu4_3)
            return out    

class Vgg16Experimental(torch.nn.Module):
    "everything exposed so you can experiment with various content and style representations"
    def __init__(self, requires_grad= False, show_progress= False):
        super().__init__()
        vgg_pretrained_features = models.vgg16(progress=show_progress, pretrained= True).features
        self.layer_names= ["relu1_1", "relu2_1", "relu2_2", "relu3_1", "relu3_2", "relu4_1", "relu4_3", "relu5_1"]
        self.content_feature_maps_index= 4
        self.style_feature_maps_indices= list(range(len(self.layer_names))) #all layers are used for style representation
        ...
        self.max_pool1= vgg_pretrained_features[4]
        ...
        self.maxpool2= vgg_pretrained_features[9]

        self.relu3_1= vgg_pretrained_features[10]
        self.relu3_2= vgg_pretrained_features[11]
        ...
        self.maxpool3= vgg_pretrained_features[16]
        ...
        self.maxpool4= vgg_pretrained_features[23]
        ...
        self.maxpool5= vgg_pretrained_features[30]
...
class Vgg19(torch.nn.Module):
    """
    Only those layers are exposed which have been used in the original NST paper
    
    conv1_1, conv2_1, conv3_1, conv4_1, conv5_1 were used for style representation
    
    conv4_2 was used for content representation (although  they did experiments with conv2_2 and conv5_2)
    ...
    def __init__(self, requires_grad= False, show_progress= False, use_relu= True):
        ...
        if use_relu:   #use relu or as in original NST paper ~ conv layers
        ...
        #all layers are used for style representation except conv4_2
        self.style_feature_maps_indices= list(range(len(self.layer_names)))
        self.style_feature_maps_indices.remove(4)  #don't need conv4_2
        ...
        for i in range(6+self.offset, 11 + self.offset):
            self.slice3.add_modules(str(i),vgg_pretrained_features[i])
        for i in range(11+self.offset, 20 + self.offset):
            self.slice4.add_modules(str(i), vgg_pretrained_features[i])
        for i in range(20+self.offset, 22):
            self.slice5.add_module(str(i),vgg_pretrained_features[i])
        for i in range(22, 29+self.offset):
            self.slice6.add_module(str(i),vgg_pretrained_features[i])
